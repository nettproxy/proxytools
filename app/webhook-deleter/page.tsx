...
function deleteWebhook(webhookUrl: string) {
...
export default function WebhookDeleter() {
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex flex-1 items-center justify-center px-4 py-12">
        <Card className="w-full max-w-md">
          <CardHeader>
            <CardTitle>Webhook Deleter</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button onClick={handleDelete} disabled={loading} className="w-full">
              {loading ? (
                ...
                <>
                  <Send className="mr-2 h-4 w-4" /> Delete Webhook
                </>
              )}
            </Button>
          </CardContent>
